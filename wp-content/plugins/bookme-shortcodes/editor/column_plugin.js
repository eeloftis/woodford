var column={
    ...
    id :'bookme-form-columns',
    pluginName: 'column'
};
(function() {
    _create_tinyMCE_options(column, 1094);
})();

function return_html_column(pluginObj){
    var form = jQuery('<div id="'+pluginObj.id+'" class="sc-container"><table id="sc-table" class="form-table">\
			<tr>\
				<th><label for="sc-no-of-columns">Number of Columns in a Row</label></th>\
				<td><select name="type" id="sc-no-of-columns" class="select2">\
            ...
            <tr id="wptwo" style="display:none;">\
				<th><label for="sc-two-columns">2 Column Division</label></th>\
				<td><select name="type" id="sc-two-columns" class="sc-change-col">\
			...
			<tr id="wpthree" style="display:none;">\
				<th><label for="sc-three-columns">3 Column Division</label></th>\
				<td><select name="type" id="sc-three-columns" class="sc-change-col">\
            ...
            <tr>\
                <td colspan="2" id="append_column_table"></td>\
            </tr>\
            <tr>\
				<th><label for="sc-column-class">Row Class:</label></th>\
				<td><input type="text" name="line" id="sc-column-class" value=""/>\
				</td>\
			</tr>\
		</table>\
		<p class="submit">\
			<input type="button" id="sc-column-submit" class="button-primary" value="Insert Columns" name="submit" />\
		</p>\
		</div>');

    return form;
}
function create_column(pluginObj){
    var form=jQuery(pluginObj.hashId);
    var table = form.find('table#sc-table');

    jQuery('#sc-no-of-columns').select2({ width : '50%' });

    function show_table(){

        var ele='',e=0,sm,smoff,md,mdoff,lg,lgoff,xs,xsoff,sel,val=0,selcol,hidecol;
        var col= form.find('#sc-no-of-columns').val();
        ...
        ele+= '<table id="appended" class="tb_multiple_column"><thead><tr><th style="width:20%">Screen</th><th style="width:50px">Hide All</th>';

        for(i=1;i<=col;i++){
            ele+='<th>Column '+i+'<br/><br/><div class="head_division">Column</div><div class="head_division">Offset</div><div class="head_division head_division_check right">Hide</div></th>';
        }
        ele+= '</tr></thead><tbody class="column_tbody">';
        jQuery.each(option,function(index,val){
            ele+='<tr><th class="column_td_first">'+val+'</th><td style="width:50px"><input type="checkbox"  name="'+index+'rowhide" id="'+index+'rowhide" value="true"></td>';
            for(var i=1;i<=col;i++){
                ...
                clearleft='<input type="checkbox"  name="'+index+'clear['+i+']" id="'+index+'clear'+i+'" value="true">';

                hidecol='<input type="checkbox"  name="'+index+'hide['+i+']" id="'+index+'hide'+i+'" value="true">';
        ...
        jQuery("#appended tbody tr:visible:even").css('background-color', '#ffffff');
        jQuery("#appended tbody tr:visible:odd").css('background-color', '#efefef');
    }
    function chnage_col_value(){

        var col= form.find('#sc-no-of-columns').val(),str,arr=[],i=0;
        if(col==2 || col==3){
            if(col==2){
                str=form.find('#sc-two-columns').val();
            } else if(col==3){
                str=form.find('#sc-three-columns').val();
            }
            arr=str.split('$');
    ...
    jQuery('#sc-no-of-columns').change(function(){
    ...
    form.find('.sc-change-col').change(function(){
        chnage_col_value();
    })
    var arr={
        1:'lg',
    ...
    form.find('#sc-column-submit').click(function(){
        ...
        var noOfColumns = jQuery('#sc-no-of-columns').val();
        var shortcode = '';
        var cusclass='';
        if(table.find('#sc-column-class').val()!=''){
            cusclass= ' class="'+table.find('#sc-column-class').val()+'"';
        }
        ...
        for(var i=1;i<=parseInt(noOfColumns);i++){
            ...
            //if(a_md[i]!=12){
            if( jQuery('#mdrowhide').is(':checked') != true )
                md=' md="'+a_md[i]+'"';
            //} else{
            //    md='';
            //}
            //if(a_sm[i]!=12){
            if( jQuery('#smrowhide').is(':checked') != true )
                sm=' sm="'+a_sm[i]+'"';
            //} else{
            //    sm='';
            //}
            //if(a_xs[i]!=12){
            if( jQuery('#xsrowhide').is(':checked') != true )
                xs=' xs="'+a_xs[i]+'"';
            //} else{
            //    xs='';
            //}
            if(a_md_off[i]!=0){
                mdoff=' offset_md="'+a_md_off[i]+'"';
            }
            else{
                mdoff='';
            }
            if(a_sm_off[i]!=0){
                smoff=' offset_sm="'+a_sm_off[i]+'"';
            }
            else{
                smoff='';
            }
            if(a_xs_off[i]!=0){
                xsoff=' offset_xs="'+a_xs_off[i]+'"';
            }
            else{
                xsoff='';
            }
            if(a_lg_off[i]!=0){
                lgoff=' offset_lg="'+a_lg_off[i]+'"';
            }
            else{
                lgoff='';
            }
